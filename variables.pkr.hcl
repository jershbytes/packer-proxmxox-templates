...
variable "efi_storage_pool" {
  description = "Name of the Proxmox storage pool to store the EFI disk on."
  type = string
  default = "zfs-storage"
}

variable "pre_enrolled_keys" {
  description = "Whether Microsoft Standard Secure Boot keys should be pre-loaded on the EFI disk."
  type = bool
  default = false
}
